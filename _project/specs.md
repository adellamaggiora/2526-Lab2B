...
dato che il grafo non è orientato e non ha self-loop avremo sempre `u<v`. 
Il peso `weight` può essere arbitrario (anche negativo). Il significato degli altri campi verrà descritto successivamente. Se necessario nella `struct` potete inserire anche altri campi.
